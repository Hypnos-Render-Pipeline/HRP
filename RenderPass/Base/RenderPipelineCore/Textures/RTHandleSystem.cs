...
namespace HypnosRenderPipeline.RenderGraph
{
    ...
    public delegate Vector2Int FScaleFunc(Vector2Int size);

    /// <summary>
    /// List of properties of the RTHandle System for the current frame.
    /// </summary>
    public struct FRTHandleProperties
    {
        /// <summary>
    ...
    /// </summary>
    public partial class FRTHandleSystem : IDisposable
    {
        ...
        EMSAASamples m_ScaledRTCurrentMSAASamples = EMSAASamples.None;
        HashSet<FRTHandle>   m_AutoSizedRTs;
        FRTHandle[]          m_AutoSizedRTsArray; // For fast iteration
        HashSet<FRTHandle>   m_ResizeOnDemandRTs;
        FRTHandleProperties m_RTHandleProperties;

        /// <summary>
        /// Current properties of the RTHandle System.
        /// </summary>
        public FRTHandleProperties rtHandleProperties { get { return m_RTHandleProperties; } }
        ...
        /// </summary>
        public FRTHandleSystem()
        {
            m_AutoSizedRTs = new HashSet<FRTHandle>();
            m_ResizeOnDemandRTs = new HashSet<FRTHandle>();
            m_MaxWidths = 1;
            m_MaxHeights = 1;
        }
        ...
        public void Initialize(int width, int height, bool scaledRTsupportsMSAA, EMSAASamples scaledRTMSAASamples)
        {
        ...
        public void Release(FRTHandle rth)
        {
            if (rth != null)
            {
        ...
        internal void Remove(FRTHandle rth)
        {
        ...
        public void SetReferenceSize(int width, int height, EMSAASamples msaaSamples)
        {
        ...
        public void SetReferenceSize(int width, int height, EMSAASamples msaaSamples, bool reset)
        {
        ...
        internal void SwitchResizeMode(FRTHandle rth, ResizeMode mode)
        {
        ...
        void DemandResize(FRTHandle rth)
        {
            ...
            if (sizeChanged || msaaSampleChanged)
            {
                ...
                // Generate a new name
                rt.name = FCoreUtils.GetRenderTargetAutoName(
                        rt.width,
                        rt.height,
                        rt.volumeDepth,
                        rt.format,
        ...
        void Resize(int width, int height, EMSAASamples msaaSamples, bool sizeChanged, bool msaaSampleChanged)
        {
            ...
            for (int i = 0, c = m_AutoSizedRTsArray.Length; i < c; ++i)
            {
                ...
                // Regenerate the name
                renderTexture.name = FCoreUtils.GetRenderTargetAutoName(renderTexture.width, renderTexture.height, renderTexture.volumeDepth, renderTexture.format, rth.m_Name, mips: renderTexture.useMipMap, enableMSAA: rth.m_EnableMSAA, msaaSamples: m_ScaledRTCurrentMSAASamples);
        ...
        /// <returns></returns>
        public FRTHandle Alloc(
            int width,
            int height,
            int slices = 1,
            EDepthBits depthBufferBits = EDepthBits.None,
            ...
            int anisoLevel = 1,
            float mipMapBias = 0f,
            EMSAASamples msaaSamples = EMSAASamples.None,
        ...
        {
            bool enableMSAA = msaaSamples != EMSAASamples.None;
            ...
            RenderTexture rt;
            if (isShadowMap || depthBufferBits != EDepthBits.None)
            {
                ...
                rt = new RenderTexture(width, height, (int)depthBufferBits, format, RenderTextureReadWrite.Linear)
                {
                    ...
                    name = FCoreUtils.GetRenderTargetAutoName(width, height, slices, format, name, mips: useMipMap, enableMSAA: enableMSAA, msaaSamples: msaaSamples)
                };
            ...
            else
            {
                ...
                rt = new RenderTexture(width, height, (int)depthBufferBits, colorFormat)
                {
                    ...
                    useDynamicScale = m_HardwareDynamicResRequested && useDynamicScale,
                    memorylessMode = memoryless,
                    name = FCoreUtils.GetRenderTargetAutoName(width, height, slices, colorFormat, name, mips: useMipMap, enableMSAA: enableMSAA, msaaSamples: msaaSamples)
                };
            }

            rt.Create();

            var newRT = new FRTHandle(this);
        ...
        // Internal function
        FRTHandle AllocAutoSizedRenderTexture(
            int width,
            int height,
            int slices,
            EDepthBits depthBufferBits,
            ...
            )
        {
            ...
            if (isShadowMap || depthBufferBits != EDepthBits.None)
            {
                ...
                rt = new RenderTexture(width, height, (int)depthBufferBits, format, RenderTextureReadWrite.Linear)
                {
                    ...
                    name = FCoreUtils.GetRenderTargetAutoName(width, height, slices, colorFormat, name, mips: useMipMap, enableMSAA: allocForMSAA, msaaSamples: m_ScaledRTCurrentMSAASamples)
                };
            }
            else
            {
        ...
        public FRTHandle Alloc(RenderTexture texture)
        {
            var rth = new FRTHandle(this);
        ...
        /// <param name="texture">Input texture</param>
        /// <returns>A new RTHandle referencing the input texture.</returns>
        public FRTHandle Alloc(Texture texture)
        {
            var rth = new FRTHandle(this);
        ...
        public FRTHandle Alloc(RenderTargetIdentifier texture)
        {
            var rth = new FRTHandle(this);
            rth.SetTexture(texture);
            rth.m_EnableMSAA = false;
        ...
        private static FRTHandle Alloc(FRTHandle tex)
        {
